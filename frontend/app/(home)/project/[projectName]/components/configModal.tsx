...
export default function ConfigModal({
    ...
    const getFileList = (fileList: Array<any>) => {
        ...
        for (let i = 0; i < fileList.length; i++) {
            if (typeof fileList[i] === "string") { tempArray.push({ name: fileList[i], type: "file" }); }
            else {
    ...
    // @ts-ignore
    const [currentFileList, setCurrentFileList] = useState<Array<File>>(getFileList(RootDir[Object.keys(RootDir)[0]]));
                ...
                {(onClose) => (
                    <>
                        <ModalBody className="w-full h-full relative left-0 top-0">
                            <div
                                className="w-full h-full absolute top-0 left-0 z-[1] p-2 overflow-scroll no-scrollbar flex flex-col gap-2 "
                            >
                                {currentFileList.map((file: File) =>
                                    <div key={file.name}>
                                        {file.type === "file" ? (
                                            <div className="w-full flex gap-2 text-md items-center cursor-pointer p-2 bg-[#33333322] hover:bg-[#33333377] transition-background rounded-lg" onClick={() => handleConfig(currentPath + "/" + file.name)}>
                                                {/* double click input */}
                                                <span>{file.name}</span>
                                            </div>
                                        ) : (
                                            <div className="w-full flex gap-2 text-md items-center cursor-pointer p-2 bg-[#33333322] hover:bg-[#33333377] transition-background rounded-lg" onClick={() => {
                                                ...
                                                setCurrentFileList((currentList) => {
                                                    ...
                                                    for (let i = 0; i < fl.length; i++) {
                                                        if (typeof fl[i] === "string") { }
                                                            ...
                                                            if (Object.keys(fl[i])[0] === file.name) return getFileList(fl[i][Object.keys(fl[i])[0]]);
                                                        }
                                                    }
                                                    return currentList;
                                                });
                                                setCurrentDir((currentDir: any) => {
                                                    ...
                                                    for (let i = 0; i < fl.length; i++) {
                                                        if (typeof fl[i] === "string") { }
                                                            ...
                                                            if (Object.keys(fl[i])[0] === file.name) return fl[i];
                                                        }
                                                    }
                                                    return currentDir;
                                                });
